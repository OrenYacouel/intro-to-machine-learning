...
def findMostCalledY(neighbors):
    return max(set(neighbors), key=neighbors.count)


def findPredByK(x, k, x_train, y_train):
    distances = list()
    counter = 0
    for neighbor in x_train:
        dist = np.sqrt(np.sum(np.square(neighbor - x)))
        distances.append((dist, y_train[counter]))
        counter += 1
    distances.sort()
    neighbors = list()
    for i in range(k):
        neighbors.append(distances[i][1])
    return findMostCalledY(neighbors)
...
def predictknn(classifier, x_test: np.array):
    ...
    for x in x_test:
        output.append(findPredByK(x, k, x_train, y_train))

    return np.reshape(output, (len(x_test), 1))
